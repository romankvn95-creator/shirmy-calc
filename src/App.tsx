...
export default function App() {
  ...
  const calcResults = useMemo(() => {
    ...
    if (isNonStandard) total *= (1 + CALC_CONFIG.nonStandardPenalty);
    ...
    return { sections, pricePerSection, finalPricePerSection: total / sections, total, prepayment, cost, margin, managerEarnings, isNonStandard, term: CALC_CONFIG.productionDays };
  }, [params]);
      ...
      <svg viewBox="0 0 400 240" className="w-full h-full drop-shadow-sm" preserveAspectRatio="xMidYMid meet">
        ...
        <g>
          ...
          {[...Array(sectionsCount)].map((_, i) => (
            <g key={i}>
              <rect x={startX + i * sectionW} width={sectionW} fill={fillColor} stroke={frameStrokeColor} y={frontY} height={totalH} strokeWidth="2" />
              {hasWheels && (
                <g>
          ...
          {hasWheels && <text x={startX + totalW + 10} y={frontY + totalH + 8} className="text-[9px] font-bold fill-indigo-600">На колёсах</text>}
          <g className="text-slate-400">
            ...
            <SVGAnimatedNumber value={params.height} x={startX - 25} y={frontY + totalH/2} textAnchor="middle" className="text-[10px] font-bold fill-slate-500" transform={`rotate(-90 ${startX - 25},${frontY + totalH/2})`} suffix=" мм" />
          </g>
        </g>
        <g>
          ...
          <SVGAnimatedNumber value={params.width} x={startX + totalW/2} y={topViewY + 48} textAnchor="middle" className="text-[9px] font-bold fill-slate-500" suffix=" мм" />
          {[...Array(sectionsCount)].map((_, i) => {
            const isBack = i % 2 === 0;
            const x1 = startX + i * sectionW, x2 = startX + (i + 1) * sectionW;
            const y1 = topViewY + (isBack ? 20 : 0), y2 = topViewY + (isBack ? 0 : 20);
            return (
              <g key={i}>
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={frameStrokeColor} strokeWidth="5" strokeLinecap="round" />
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={fillColor} strokeWidth="3" strokeLinecap="round" />
                <circle cx={x1} cy={y1} fill={frameStrokeColor} r="2.5" />
                {i === sectionsCount - 1 && <circle cx={x2} cy={y2} fill={frameStrokeColor} r="2.5" />}
                {i === 0 && <text x={x1 + sectionW/2} y={y1 + (isBack ? -10 : 30)} textAnchor="middle" className="text-[8px] font-bold fill-slate-400">60 см</text>}
              </g>
            );
          })}
        </g>
  ...
  return (
    <div className="flex flex-col min-h-screen lg:h-screen lg:overflow-hidden bg-[#F8FAFC] text-slate-900 font-sans">
      <header className="h-16 border-b border-slate-200 bg-white px-6 flex items-center justify-between z-10 shadow-sm shrink-0">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="font-bold text-xs sm:text-sm md:text-lg leading-tight tracking-tight text-slate-900">Калькулятор ширм и перегородок</h1>
      ...
      <main className="flex-1 p-4 md:p-6 grid grid-cols-12 auto-rows-min gap-4 md:gap-6 overflow-y-auto custom-scrollbar">
        ...
        <section className="order-1 col-span-12 lg:col-span-4 lg:row-start-1 lg:row-end-7 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col h-fit">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <h2 className="font-bold text-sm md:text-base flex items-center gap-2">
              <span className="w-1.5 h-4 bg-indigo-500 rounded-full"></span>Конфигурация
            </h2>
            <RefreshCw size={14} className="text-slate-400 cursor-pointer hover:rotate-180 transition-transform duration-500" onClick={() => setParams({ width: 2000, height: 1800, color: 'Белый', frame: 'Дерево', customer: '' })} />
          </div>
          <div className="p-5 space-y-5">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Имя заказчика</label>
              <input type="text" value={params.customer} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-indigo-500 transition-colors" placeholder="Введите имя..." onChange={(e) => setParams(prev => ({ ...prev, customer: e.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              {(['width', 'height'] as const).map(dim => (
                <div key={dim} className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{dim === 'width' ? 'Ширина проёма (мм)' : 'Высота секции (мм)'}</label>
                    <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded"><AnimatedNumber value={params[dim]} /></span>
                  </div>
                  <div className="space-y-3">
                    <input type="text" inputMode="numeric" value={params[dim] === 0 ? '' : params[dim]} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-indigo-500 transition-colors" placeholder={dim === 'width' ? '2000' : '1800'} onChange={(e) => handleNumChange(dim, e.target.value)} />
                    <div className="px-1">
                      <input type="range" min={dim === 'width' ? 600 : 1000} max={dim === 'width' ? 6000 : 3000} step="10" value={params[dim]} onChange={(e) => setParams(prev => ({ ...prev, [dim]: Number(e.target.value) }))} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {CALC_CONFIG.frameMaterials.map(frame => (
                  <button key={frame.id} onClick={() => setParams(prev => ({ ...prev, frame: frame.name }))} className={`px-3 py-2 text-[10px] font-bold rounded-xl border transition-all ${params.frame === frame.name ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-slate-50 border-slate-200 text-slate-600 hover:border-indigo-300'}`}>{frame.name}</button>
                ))}
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {CALC_CONFIG.colors.map(color => (
                  <button key={color} onClick={() => setParams(prev => ({ ...prev, color }))} className={`px-4 py-2 text-xs font-bold rounded-xl border transition-all ${params.color === color ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-slate-50 border-slate-200 text-slate-600 hover:border-indigo-300'}`}>{color}</button>
                ))}
              </div>
            </div>
            <div className="bg-amber-50/50 border border-amber-100 rounded-xl p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] font-bold text-amber-700 uppercase">Расчёт секций:</span>
              ...
              {calcResults.isNonStandard && <div className="mt-2 text-[9px] text-amber-800 font-bold bg-amber-200/50 px-2 py-1 rounded inline-block">+{CALC_CONFIG.nonStandardPenalty * 100}% за нестандартную ширину проёма</div>}
            </div>
          </div>
        </section>

        {/* Drawing Card */}
        <div className="order-2 col-span-12 lg:col-span-5 lg:row-start-1 lg:row-end-5 bg-white border border-slate-200 rounded-3xl shadow-sm p-6 flex flex-col relative overflow-hidden group h-fit lg:h-auto min-h-[300px]">
          ...
          <div className="flex-1 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 flex items-center justify-center p-6 relative">
            ...
            <div className="relative z-0 w-full h-full max-w-md"><RenderDrawing /></div>
          </div>
          <div className="absolute bottom-6 right-6">
            <button className="p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-500 hover:text-indigo-600 transition-all shadow-sm active:scale-95"><Download size={18} /></button>
          </div>
        </div>

        {/* Result Card */}
        <div className="order-3 col-span-12 lg:col-span-3 lg:row-start-1 lg:row-end-5 bg-slate-900 rounded-[32px] shadow-2xl shadow-slate-300 p-8 flex flex-col text-white relative overflow-hidden h-fit lg:h-auto">
          ...
          <div className="mb-auto relative z-10">
            <div className="text-slate-400 text-[10px] uppercase tracking-[0.2em] font-bold mb-2">Общая стоимость</div>
            <div className="flex items-baseline gap-2">
              <span className="text-5xl md:text-6xl font-black tracking-tight leading-none"><AnimatedNumber value={calcResults.total} /></span>
            ...
            <div className="mt-6 flex flex-col gap-2 pt-6 border-t border-white/10">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-black text-indigo-400"><AnimatedNumber value={calcResults.prepayment} suffix=" ₽" /></span>
              </div>
            </div>
            {mode === 'manager' && (
              <div className="mt-4 space-y-2 pt-4 border-t border-white/5">
                <div className="flex justify-between text-[10px] text-slate-400 uppercase font-black">
                  <span>Цена за секцию:</span><span className="text-white"><AnimatedNumber value={calcResults.pricePerSection} suffix=" ₽" /></span>
                </div>
                <div className="flex justify-between text-[10px] text-slate-400 uppercase font-black pt-2">
                  <span>Себестоимость:</span><span className="text-slate-300"><AnimatedNumber value={calcResults.cost} suffix=" ₽" /></span>
                </div>
                <div className="flex justify-between text-[10px] text-emerald-400 uppercase font-black">
                  <span>Маржа:</span><span><AnimatedNumber value={calcResults.margin} suffix=" ₽" /></span>
                </div>
                <div className="flex justify-between text-[10px] text-orange-400 uppercase font-black mt-2 bg-white/5 p-2 rounded-lg border border-orange-500/20">
                  <span>Заработок (20%):</span><span><AnimatedNumber value={calcResults.managerEarnings} suffix=" ₽" /></span>
                </div>
              </div>
            )}
          </div>
          <div className="space-y-4 pt-8 relative z-10">
            <div className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/10">
              <div className="w-10 h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center text-indigo-400"><RefreshCw size={20} /></div>
            ...
            <button onClick={() => setShowKP(true)} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl shadow-xl transition-all active:scale-[0.98] text-sm tracking-wide">Сформировать КП</button>
          </div>
        </div>
        ...
        <div className="order-4 lg:order-5 col-span-12 lg:col-span-3 lg:row-start-5 lg:row-end-7 bg-white border border-slate-200 rounded-3xl shadow-sm p-5 h-fit">
          ...
          <div className="space-y-2.5">
            {[
              { label: 'Материал каркаса', value: params.frame, isText: true },
              { label: 'Цена за секцию', value: calcResults.pricePerSection, suffix: ' ₽' },
              { label: 'Кол-во секций', value: calcResults.sections, suffix: ' шт' },
              { label: `Предоплата ${CALC_CONFIG.prepaymentRate * 100}%`, value: calcResults.prepayment, suffix: ' ₽', highlight: true },
            ].map(({ label, value, suffix, isText, highlight }) => (
              <div key={label} className="flex justify-between items-center text-xs">
                <span className="text-slate-500">{label}</span>
                <span className={`font-bold ${highlight ? 'text-indigo-600' : 'text-slate-700'}`}>
            ...
            <div className="flex justify-between items-center text-xs font-bold text-slate-900 pt-1">
              <span>Итого к оплате</span><span><AnimatedNumber value={calcResults.total} suffix=" ₽" /></span>
            </div>
          </div>
        </div>
        ...
        <div className="order-5 lg:order-4 col-span-12 lg:col-span-5 lg:row-start-5 lg:row-end-7 bg-white border border-slate-200 rounded-3xl shadow-sm p-5 overflow-hidden h-fit">
          <h3 className="font-bold text-slate-400 uppercase mb-4 text-[10px] tracking-wider">Примеры из портфолио</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[300px] lg:max-h-[140px] overflow-y-auto custom-scrollbar pr-1">
            {demoGallery.map(item => (
              <div key={item.id} onClick={() => setSelectedImage(item)} className="flex gap-3 items-center p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 transition-colors cursor-pointer group">
      ...
      {/* Lightbox */}
      {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 bg-slate-950/90 backdrop-blur-sm" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-4xl w-full bg-white rounded-3xl overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/20 hover:bg-black/40 text-white rounded-full flex items-center justify-center backdrop-blur-md transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-6 md:p-10 flex flex-col justify-center">
                ...
                <button onClick={() => setSelectedImage(null)} className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-colors">Закрыть просмотр</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showKP && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-md overflow-hidden">
          <div className="bg-white w-full max-w-2xl h-full max-h-[95vh] sm:max-h-[90vh] rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-5 sm:p-10 space-y-6 sm:space-y-8 custom-scrollbar">
              ...
              <div className="bg-slate-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-100">
                ...
                <div className="aspect-[2/1] w-full bg-white rounded-lg sm:rounded-xl border border-slate-200 p-2 sm:p-4 shadow-inner"><RenderDrawing /></div>
              </div>
            ...
            <div className="p-4 sm:p-8 border-t border-slate-100 bg-slate-50 flex flex-col sm:flex-row gap-3">
              <button onClick={() => window.print()} className="flex-[2] py-3 sm:py-4 bg-indigo-600 text-white font-bold rounded-xl sm:rounded-2xl flex items-center justify-center gap-2 hover:bg-indigo-500 transition-all active:scale-95">
                <Download size={18} /> Сохранить (PDF)
              </button>
              <button onClick={() => setShowKP(false)} className="flex-1 py-3 sm:py-4 bg-white text-slate-900 font-bold rounded-xl sm:rounded-2xl border border-slate-200 hover:bg-slate-100 transition-colors">Вернуться</button>
            </div>
          </div>
        </div>
      )}
